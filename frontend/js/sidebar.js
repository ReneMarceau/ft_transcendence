    ...
    for (let i = 0; i < friendList.length; i++) {
        ...
        <li class="nav-item" style="margin: 0px;">
            <div class="row align-items-center">
                <div class="col-auto">
                    <a href="/profile?userid=${friendList[i]}" class="nav-link" data-link>
                        ${username}
...
async function initAddFriends()
{
    ...
    addFriendBtn.addEventListener("click", async () => {
        const friendList = await getFriendList(userid)
        if (friendList.includes(addFriendInput.value)) {
    ...
    removeFriendButtons.forEach(button => {
        button.addEventListener("click", async () => {
            ...
            const response = await fetch(`/api/profiles/${userid}/`, {
                ...
                body: JSON.stringify({
                    friends: newFriendList // Assuming your API endpoint knows how to remove a specific friend by ID
                }),
                credentials: 'same-origin'
            });
            ...
            if (response.ok) {
                ...
                console.log(responseData);
                createAlert('success', responseData.detail);
                initSideBar(); // Refresh the sidebar to reflect the changes
            } else {
                const errorData = await response.json();
                createAlert('danger', errorData.detail);
                console.error('Error:', errorData);
