...
class Enable2FAView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if hasattr(user, "is_2fa_enabled") and user.is_2fa_enabled:
            return Response(
                {"detail": "2FA is already enabled for this user."}, status=400
            )

        # Check if a TOTP device already exists
        if TOTPDevice.objects.filter(user=user, name="default").exists():
            return Response(
                {"detail": "A TOTP device already exists for this user."}, status=400
            )
...
class Disable2FAView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            return Response(
                {"detail": "No TOTP device found for this user."}, status=400
            )
